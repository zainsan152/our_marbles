...
<form  action="@if(isset($product)) {{route('product.update', $product)}}
    @else {{route('product.store')}} @endif" method="post" accept-charset="utf-8" enctype="multipart/form-data">
    <div class="row">
        @csrf
        @if(isset($product))
        @method('PUT')
        ...
        <div class="form-group row">
            <div class="col-lg-12">
                <label class="form-control-label">Shopkeeper: </label>
                <textarea name="shopkeeper" id="editor" class="form-control ">{!! @$product->shopkeeper !!}</textarea>
            </div>
        </div>
        <div class="form-group row">
            <div class="col-6">
